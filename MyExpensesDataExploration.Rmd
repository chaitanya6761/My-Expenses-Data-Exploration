...
cat("Total Number Of Atm Visits: ", nrow(totalAtmVisits_2017))
cat("\nTotal Amount of Money Withdrawn: ",sum(totalAtmVisits_2017$AMOUNT))
```

Now lets compare the above 2017 atm stats to 2016 atm stats to see whether **Demonitisation** played any role in number of atm withdrawls.
```{r}
totalAtmVisits_2016 <- subset(expensesTemp,expensesTemp$PLACE == "ATM" & expensesTemp$YEAR == "2016")
cat("Total Number Of Atm Visits: ", nrow(totalAtmVisits_2016))
cat("\nTotal Amount of Money Withdrawn: ",sum(totalAtmVisits_2017$AMOUNT))
```

The above stats clearly show that demonitisation played a role in decreasing my number of atm withdraws and increasing my card spends.
